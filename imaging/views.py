...
class CaptureUploadView(APIView):
    ...
    def post(self, request):
        ...
        # Create alert if garbage detected
        if "Garbage Detected" in result["label"]:
            try:
                from alerts.models import GarbageAlert
                GarbageAlert.objects.create(
                    classification=classification,
                    message=f"Garbage detected by {device} — {result['label']} ({result['confidence']:.1%} confidence)",
                )
            except Exception as e:
                pass  # Don't break capture if alert fails
        ...
        return Response(
            {
                ...
                "status": result["status"],
                "alert_created": "Garbage Detected" in result["label"],
            },
